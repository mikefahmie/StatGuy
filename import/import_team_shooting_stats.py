...
print("🏀 Starting Team Season Shooting Stats Import...")
...
print("\n❓ Clear existing data in team_season_shooting_stats table first?")
...
if clear_choice == "1":
    ...
    print("   DELETE FROM team_season_shooting_stats;")
    print("\n   Press Enter when done...")
    input()

# Find all CSV files in team_season_shooting_stats folder
csv_folder = 'team_season_shooting_stats'
...
for csv_file in csv_files:
    ...
    try:
        ...
        # Remove duplicates - keep first instance of each (season, team_id)
        initial_count = len(df)
        df = df.drop_duplicates(subset=['season', 'team_id'], keep='first')
        ...
        for i in range(0, len(records), batch_size):
            ...
            try:
                supabase.table('team_season_shooting_stats').insert(batch).execute()
                ...
                for record in batch:
                    try:
                        supabase.table('team_season_shooting_stats').insert([record]).execute()
                        file_inserted += 1
                        total_inserted += 1
                    except Exception as record_error:
                        file_failed += 1
                        team_info = f"{record.get('team', 'Unknown')}"
                        error_msg = str(record_error)[:80]
                        print(f"      ❌ {team_info}: {error_msg}")
...
print("\n🔍 Verifying import...")
try:
    result = supabase.table('team_season_shooting_stats').select('season', count='exact').execute()
    db_count = result.count
    print(f"✅ Database now contains {db_count:,} team-season shooting records")
    
    # Show season breakdown
    print("\n📊 Records by season:")
    for year in range(2014, 2026):
        count_result = supabase.table('team_season_shooting_stats').select('season', count='exact').eq('season', year).execute()
